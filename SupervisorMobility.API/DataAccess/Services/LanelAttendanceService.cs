...
namespace SupervisorMobility.API.DataAccess.Services
{
    ...
    public class LanelAttendanceService : BackgroundService
    {
        private readonly ILogger<LanelAttendanceService> _logger;
        private readonly IServiceScopeFactory _serviceScopeFactory;
     

        public LanelAttendanceService(IServiceScopeFactory serviceScopeFactory, ILogger<LanelAttendanceService> logger)
        {
        ...
        protected override async Task ExecuteAsync(CancellationToken stoppingToken)
        {
            ...
            while (!stoppingToken.IsCancellationRequested)
            {
                ...
                using (var scope = _serviceScopeFactory.CreateScope())
                {
                    ...
                    var _env = serviceProvider.GetService<IWebHostEnvironment>();
                    ...
                    if (IsWithinTimeInterval(currentTime, TimeSpan.FromHours(6.25), TimeSpan.FromHours(9.25)))
                    {
                        ...
                        Execute(_assyChartService, _mapper, _env);
        ...
        public async void Execute(IAssyChartService _assyChartService, IMapper _mapper, IWebHostEnvironment _env)
        {
            string filePath = @"";
            if (_env.IsDevelopment())
            {
                filePath = Directory.GetCurrentDirectory().ToString() + "\\uploads\\attendance\\attendance.csv";
            }
            else { 
                filePath = @"C:\LenelInfo\attendance.csv";
            }
            ...
            if (File.Exists(filePath))
            {
                ...
                {
                    // Manejar cualquier excepción que pueda ocurrir al abrir o leer el archivo
